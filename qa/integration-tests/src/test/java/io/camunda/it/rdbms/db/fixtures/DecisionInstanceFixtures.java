  ...
  public static DecisionInstanceDbModel createRandomized(
    ...
    final var builder =
        new DecisionInstanceDbModel.Builder()
            ...
            .evaluatedInputs(randomInputs(decisionInstanceKey))
            .evaluatedOutputs(randomOutputs(decisionInstanceKey));

    return builderFunction.apply(builder).build();
  }

  private static List<EvaluatedInput> randomInputs(final Long key) {
    return IntStream.range(0, RANDOM.nextInt(5) + 5)
        .boxed()
        .map(i -> new EvaluatedInput(key, "input-" + i, "Input " + i, Integer.toString(i)))
        .toList();
  }

  private static List<EvaluatedOutput> randomOutputs(final Long key) {
    return IntStream.range(0, RANDOM.nextInt(5) + 5)
        .boxed()
        .map(
            i ->
                new EvaluatedOutput(
                    key, "input-" + i, "Input " + i, Integer.toString(i), "rule-" + i, i))
        .toList();
  }
